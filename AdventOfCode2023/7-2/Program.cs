...
public class Hand : IComparable<Hand>
{
    ...
    public int Strength
    {
        get
        {
            ...
            var grouped = this.Cards.GroupBy(c => c).ToDictionary(g => g.Key, g => g.Count());
            int jokerCount = grouped.ContainsKey('J') ? grouped['J'] : 0;
            
            var rankCounts = this.Cards.GroupBy(c => c).Where(g => g.Key!='J').Select(g => g.Count()).OrderByDescending(x => x).ToList();
            if (!rankCounts.Any())
            {
                return 7;
            }
